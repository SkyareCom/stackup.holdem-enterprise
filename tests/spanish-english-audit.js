...
// English is intentional only in these approved ENTERPRISE brand/template phrases.
// All other rendered UI remains subject to the strict Spanish-residue audit below.
const intentionalEnglishBranding=new Set([
  'TOURNAMENT MANAGEMENT SYSTEM',
  'STAFF / DEALER / PLAYER STATIONS'
]);
...
for(const file of walk(ROOT)){
  ...
  for(const raw of extract(src)){
    const source=clean(raw);
    if(source.length<2||source.length>360||!/\p{L}/u.test(source)||/^[A-Z0-9]+_[A-Z0-9_]+$/.test(source))continue;
    candidates++;
    if(intentionalEnglishBranding.has(source))continue;
    const es=clean(i18n.translateString(source,'es'));
    const words=(es.toUpperCase().match(/[A-ZÁÉÍÓÚÜÑ-]+/g)||[]).filter(w=>suspicious.has(w)&&!allowed.has(w));
    if(words.length)failures.push(`${rel} :: ${source} => ${es} :: ENGLISH=[${[...new Set(words)].join(', ')}]`);
  }
}

const unique=[...new Set(failures)];
if(unique.length){
  console.error(`SPANISH RENDERED-UI AUDIT FALHOU: ${unique.length} PROBLEMA(S) EM ${candidates} STRING(S) VISÍVEIS.`);
  unique.slice(0,1000).forEach(x=>console.error(' - '+x));
  process.exit(1);
}
console.log(`SPANISH RENDERED-UI AUDIT OK: ${candidates} STRING(S) VISÍVEIS VERIFICADAS, SEM INGLÊS INDEVIDO; FRASES DE MARCA APROVADAS PRESERVADAS.`);
