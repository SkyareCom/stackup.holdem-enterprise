...
function createManager(hostId,kind){
 ...
 function forcedNoAlert(id){return kind==='spoken'&&REGISTRATION_IDS.has(id)}
 function test(group,noAlert=false){const id=selected[group]||activeSelected[group];if(!id)return;const c=cfg(),l=lang();if(kind==='spoken'){const opt={lang:l,profileId:c.voiceProfile,repeat:c.voiceRepeat,volume:c.voiceVolume/100};if(noAlert||forcedNoAlert(id))api.speak(textFor(id),opt);else api.playAlertThenSpeak(textFor(id),opt)}else api.setAnnouncement(textFor(id))}
 function activate(group,noAlert=false){const id=selected[group];if(!id)return;api.toggle(kind,id,true);if(kind==='spoken'&&api.setSpokenNoAlert)api.setSpokenNoAlert(id,forcedNoAlert(id)||!!noAlert);selected[group]=null;open[group].activate=false;open[group].noAlert=false;open[group].active=true;render()}
 ...
 function messageButton(id,isSelected,disabled,pickKind,group){const note=forcedNoAlert(id)?'<em>SEMPRE SEM ALERTA</em>':'';return `<button type="button" class="msgPick ${isSelected?'selected':''}" data-pick="${pickKind}" data-group="${group}" data-id="${id}" ${disabled?'disabled':''} aria-pressed="${isSelected?'true':'false'}"><span class="msgPickText"><b>${labelFor(id)}</b><small>${textFor(id)}</small>${note}</span></button>`}
 function block(g){
  ...
  const currentId=selected[g.id],noAlertMode=kind==='spoken'&&(s.noAlert||forcedNoAlert(currentId));
  const actionButtons=kind==='spoken'?(noAlertMode?`<div class="msgPanelActions"><button type="button" data-msg-action="testNoAlert" data-group="${g.id}" ${!currentId?'disabled':''}>TESTAR MENSAGEM SEM ALERTA</button><button type="button" data-msg-action="confirmNoAlert" data-group="${g.id}" ${!currentId?'disabled':''}>ATIVAR MENSAGEM SEM ALERTA</button></div>`:`<div class="msgPanelActions"><button type="button" data-msg-action="test" data-group="${g.id}" ${!currentId?'disabled':''}>TESTAR MENSAGEM</button><button type="button" data-msg-action="confirm" data-group="${g.id}" ${!currentId?'disabled':''}>ATIVAR MENSAGEM</button></div>`):`<div class="msgPanelActions"><button type="button" data-msg-action="test" data-group="${g.id}" ${!currentId?'disabled':''}>TESTAR MENSAGEM</button><button type="button" data-msg-action="confirm" data-group="${g.id}" ${!currentId?'disabled':''}>ATIVAR MENSAGEM</button></div>`;
  const activateList=s.activate?`<div class="msgPanel"><div class="msgModeTitle">${noAlertMode?'ATIVAR MENSAGEM SEM ALERTA':'ATIVAR MENSAGEM'}</div>${availableRows.length?`<div class="msgList">${availableRows.map(r=>messageButton(r[0],currentId===r[0],false,'new',g.id)).join('')}</div>${actionButtons}`:`<div class="msgEmpty">TODAS AS MENSAGENS DESTE BLOCO JÁ ESTÃO ATIVAS.</div>`}</div>`:'';
  ...
  const controls=kind==='spoken'?`<div class="msgGroupButtons msgGroupButtonsVoice"><button type="button" data-msg-action="activateOpen" data-group="${g.id}">ATIVAR MENSAGEM</button><button type="button" data-msg-action="activateNoAlertOpen" data-group="${g.id}">ATIVAR MENSAGEM SEM ALERTA</button><button type="button" data-msg-action="activeOpen" data-group="${g.id}">MENSAGENS ATIVAS${activeRows.length?' • '+activeRows.length:''}</button></div>`:`<div class="msgGroupButtons"><button type="button" data-msg-action="activateOpen" data-group="${g.id}">ATIVAR MENSAGEM</button><button type="button" data-msg-action="activeOpen" data-group="${g.id}">MENSAGENS ATIVAS${activeRows.length?' • '+activeRows.length:''}</button></div>`;
  return `<section class="msgGroup"><button type="button" class="msgCategoryButton active" data-category="${g.id}">${g.title}</button><div class="msgCategoryBody">${controls}${activateList}${activeList}</div></section>`
 }
 function bind(){
  ...
  host.querySelectorAll('[data-msg-action]').forEach(b=>b.onclick=e=>{e.stopPropagation();const g=b.dataset.group,a=b.dataset.msgAction;if(a==='activateOpen'){open[g].activate=!open[g].activate;open[g].noAlert=false;if(open[g].activate)open[g].active=false;selected[g]=null}else if(a==='activateNoAlertOpen'){open[g].activate=!open[g].activate||!open[g].noAlert;open[g].noAlert=true;if(open[g].activate)open[g].active=false;selected[g]=null}else if(a==='activeOpen'){open[g].active=!open[g].active;if(open[g].active)open[g].activate=false;activeSelected[g]=null;open[g].edit=false;open[g].deleteArmed=false}else if(a==='test')test(g,false);else if(a==='testNoAlert')test(g,true);else if(a==='confirm')activate(g,false);else if(a==='confirmNoAlert')activate(g,true);else if(a==='edit'){open[g].edit=!open[g].edit;activeSelected[g]=null;open[g].deleteArmed=false}else if(a==='delete'){if(activeSelected[g])open[g].deleteArmed=true}else if(a==='deleteConfirm')erase(g);render()})
 }
 function render(){host.innerHTML=groups.map(block).join('');bind()}
 render();
 return {render};
}
const style=document.createElement('style');style.textContent=`#spokenMessages,#writtenMessages{display:grid;gap:8px}.msgGroup{border:0!important;border-radius:0!important;background:transparent!important;padding:0!important}.msgCategoryButton{width:100%!important;text-align:left!important;padding:12px!important}.msgCategoryButton.active{background:#0B100D!important;color:#FFFFFF!important}.msgCategoryBody{padding:10px 0 14px}.msgGroupButtons,.msgPanelActions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.msgGroupButtonsVoice{grid-template-columns:repeat(3,minmax(0,1fr))}.msgPanel{margin-top:10px;padding-top:10px;border:0!important;background:transparent!important}.msgModeTitle{color:#FFFFFF!important;margin-bottom:8px}.msgList{display:grid;gap:7px}.msgPick{display:block!important;width:100%!important;text-align:left!important;background:#020302!important;border:0!important;border-radius:0!important;padding:10px!important;box-shadow:none!important}.msgPick.selected{background:#0B100D!important}.msgPickText{display:grid;gap:4px}.msgPickText b{color:#FFFFFF!important}.msgPickText small,.msgPickText em{color:#AEB8B1!important;line-height:1.35!important;font-weight:300!important}.msgPickText em{font-style:normal;color:#8DFC3B!important}.msgPanelActions{margin-top:9px}.msgActiveHead{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:8px;color:#AEB8B1!important}.msgEmpty{color:#AEB8B1!important;padding:8px 2px}@media(max-width:700px){.msgGroupButtons,.msgGroupButtonsVoice,.msgPanelActions{grid-template-columns:1fr}.msgActiveHead{align-items:stretch;flex-direction:column}}`;document.head.appendChild(style);
